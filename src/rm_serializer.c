/**
 * @file rm_serializer.c
 * @authors Xingli Li
 * @date 2023-07-29
 * @brief Record, schema, and value serialization utilities.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
serializeRecord(Record *record, Schema *schema) {
    ...
    for (i = 0; i < schema->numAttr; i++) {
        char* serializeAttrStr = serializeAttr(record, schema, i);
        APPEND_STRING(result, serializeAttrStr);
        free(serializeAttrStr);
        APPEND(result, "%s", (i == 0) ? "" : ",");
    }
