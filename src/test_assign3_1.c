/**
 * @file test_assign3_1.c
 * @authors Xingli Li
 * @date 2023-07-29
 * @brief Record manager regression and scan tests.
 */
#include <stdlib.h>
#include "dberror.h"
#include "expr.h"
...
testCreateTableAndInsert (void)
{
	...
	TEST_CHECK(openTable(table, "test_table_r"));
	ASSERT_EQUALS_INT(numInserts, getNumTuples(table),
			"tuple count survives close and reopen");
	...
	{   TEST_CHECK(createRecord(&r, schema));  // added Fall 2021
		...
		lr = fromTestRecord(schema, inserts[pos]);
		ASSERT_EQUALS_RECORDS(lr, r, schema, "compare records");
		freeRecord(lr);
...
testUpdateTable (void)
{
	...
	TEST_CHECK(closeTable(table));
	TEST_CHECK(openTable(table, "test_table_r"));
	ASSERT_EQUALS_INT(numFinal, getNumTuples(table),
			"live tuple count survives deletes and reopen");
	...
	for(i = 0; i < numFinal; i++)
	{
		RID rid = rids[i];
		TEST_CHECK(getRecord(table, rid,r));
		lr = fromTestRecord(schema, finalR[i]);
		ASSERT_EQUALS_RECORDS(lr, r, schema, "compare records");
		freeRecord(lr);
	}
...
testInsertManyRecords(void)
{
	...
	for(i = 0; i < numInserts; i++)
	{
		RID rid = rids[i];
		TEST_CHECK(getRecord(table, rid,r));
		lr = fromTestRecord(schema, realInserts[i]);
		ASSERT_EQUALS_RECORDS(lr, r, schema, "compare records");
		freeRecord(lr);
	}
    freeRecord(r);   // Added Summer 2021
	...
	lr = fromTestRecord(schema, updates[0]);
	ASSERT_EQUALS_RECORDS(lr, r, schema, "compare records");
	freeRecord(lr);
...
void testScans (void)
{
	...
	while ((rc = next(sc,r)) == RC_OK) {
		for (i = 0; i < scanSizeOne; i++) {
			lr = fromTestRecord(schema, scanOneResult[i]);
			if (memcmp(lr->data, r->data, getRecordSize(schema)) == 0)
				foundScan[i] = TRUE;
			freeRecord(lr);
		}
	}
...
void testScansTwo (void)
{
	...
	TEST_CHECK(startScan(table, sc, sel));
	while ((rc = next(sc,r)) == RC_OK) {
		lr = fromTestRecord(schema, inserts[1]);
		ASSERT_EQUALS_RECORDS(lr, r, schema, "compare records");
		freeRecord(lr);
	}
    freeRecord(r);
	...
	TEST_CHECK(startScan(table, sc, sel));
	while ((rc = next(sc,r)) == RC_OK) {
		lr = fromTestRecord(schema, inserts[5]);
		ASSERT_EQUALS_RECORDS(lr, r, schema, "compare records");
		char *tmpStr = serializeRecord(r, schema);
		free(tmpStr);
		freeRecord(lr);
	}
	if (rc != RC_RM_NO_MORE_TUPLES)
	...
	while ((rc = next(sc,r)) == RC_OK) {
		...
		free(tmpStr);
		for (i = 0; i < numInserts; i++) {
			lr = fromTestRecord(schema, inserts[i]);
			if (memcmp(lr->data, r->data, getRecordSize(schema)) == 0)
				foundScan[i] = TRUE;
			freeRecord(lr);
		}
